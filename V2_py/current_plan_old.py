...
def show_current_plan():
    ...
    if config_filename and 'loaded_plan' not in st.session_state:
        try:
            ...
            # Populate session_state with target players from loaded plan
            for pick in loaded_plan:
                round_num = pick.get('round')
                target_player = pick.get('target_player', '')
                if round_num:
                    st.session_state[f'plan_round_{round_num}'] = target_player
    ...
    with tab_build:
        ...
        if st.session_state['hypothetical_picks']:
            ...
            with col_save:
                if config_filename and st.button("💾 Save Plan to Config", use_container_width=True):
                    # Convert hypothetical picks to plan format
                    plan = []
                    for round_num in sorted(hyp_picks.keys()):
                        pick = hyp_picks[round_num]
                        plan.append({
                            'round': round_num,
                            'position': pick['position'],
                            'target_player': pick['player_name'],
                            'status': 'ACTIVE'
                        })
                    
                    ConfigManager.save_plan(config_filename, plan)
                    st.success(f"✅ Plan saved! {len(plan)} rounds configured.")
                elif not config_filename:
                    st.warning("No config file loaded. Cannot save plan.")
                st.rerun()
